...
int networkMsgSend (int sock, char *msgP, int msgLen, char *host, int port) {
  ...
  if (sendto
      (sock, msgP, msgLen, 0, (struct sockaddr *) &address,
       sizeof (address)) < 0) {
...
int networkMsgRecv (int sock, char *msgP, int msgLen, struct sockaddr_in *address) {
  ...
  dataLen =
    recvfrom (sock, msgP, msgLen, 0, (struct sockaddr *) address, &addrLen);
  if (dataLen < 0) {
...
int SendMsg (
    osip_transaction_t *tr, osip_message_t *sip, char *host, int port,
    int out_socket) {
  int len = 0;
  int status;
  ...
  char *msgP;
  int msgLen;
  int i = osip_message_to_str(sip, &msgP, &msgLen);
  if (i != 0) {
    OSIP_TRACE (osip_trace
                (__FILE__, __LINE__, OSIP_BUG, NULL,
                 "failed to convert message/n"));
    return -1;
  }
  if (!networkMsgSend (sipSock, msgP, strlen (msgP), host, port))
    OSIP_TRACE (osip_trace
                (__FILE__, __LINE__, OSIP_INFO1, NULL,
                 "Time: Udp message sent: /n%s/n", msgP));
...
void setCallbacks (osip_t *osip) {
  ...
  osip_set_message_callback(osip, OSIP_ICT_STATUS_1XX_RECEIVED,
                             cb_rcvICTRes);
  osip_set_message_callback(osip, OSIP_NICT_STATUS_1XX_RECEIVED,
                             cb_rcvNICTRes);
...
int bSipSend (osip_message_t *msgPtr, osip_fsm_type_t transactionType) {
  ...
  if ((status =
       osip_transaction_init (&transactionPtr, transactionType, osip,
                              msgPtr)) != 0) {
  ...
  if ((status =
       osip_transaction_add_event (transactionPtr, sipeventPtr)) != 0) {
...
int bSipRegisterBuild (osip_message_t ** regMsgPtrPtr) {
  ...
  if ((status = osip_message_init (B.MsgPtr)) != 0) {
      OSIP_TRACE (osip_trace
                  (__FILE__, __LINE__, OSIP_BUG, NULL,
                   "Can't init message!/n"));
      return -1;
  }
  ...
  if ((status = osip_call_id_init (&callidPtr)) != 0) {
      OSIP_TRACE (osip_trace
                  (__FILE__, __LINE__, OSIP_BUG, NULL, "call id failed!/n"));
      osip_message_free(regMsgPtr);
      return -1;
  }
  ...
  if ((status = osip_cseq_init (&cseqPtr)) != 0) {
      OSIP_TRACE (osip_trace
                  (__FILE__, __LINE__, OSIP_BUG, NULL, "seq init failed!/n"));
      osip_message_free(regMsgPtr);
      return -1;
  }
  gSeqNum++;
  ...
  sprintf(temp, "SIP/2.0/%s %s;branch=z9hG4bK%u", "UDP", LOCAL_IP,
...
int bSipRegister (void *cookie) {
  ...
  if (bSipRegisterBuild (B.MsgPtr) != 0) {
